...
<template>
	<div>
		<stree :data="jobTree" :curr="curr" :menu="grade_edit" @item-click="itemClick" @menu-click="menuClick" :hideStaffNums="hideStaffNums"></stree>
	...
	export default {
		...
		methods: {
			...
			loadTree(callback) {
				...
				loadJobTree(data).then(valid.call(this)).then((res) => {
					if(res.ok) {
			...
			itemClick(item) {
				this.$emit('onItemClick', item, 'grade');
