...
<template >
	<div class="resource-new-gsx">
		...
		<case-bar1 v-if="!isBranchOfficeLeader" title="分公司" :isShow='false' :tagList='branchOfficelists' @addAcitveCon='addAcitveGroup'  :num='num1' ></case-bar1>
		...
		<!-- <form class="formexport" :action="exportUrl" method="post" target="_blank">
			...
			<input type="hidden" name="createType" v-model="createType" />
			...
			<input type="hidden" name="phase" v-model="phase" />
			<input type="hidden" name="pageNo" v-model="pageNo" />
			<input type="hidden" name="pageSize" v-model="pageSize" />
			<input type="hidden" name="officeId" v-model="companyId" />
		</form> -->
	</div>
</template>
...
export default {	
	...
	computed: {
		...mapGetters('crm', ['isNew', 'isBranchOfficeLeader']),
		// exportUrl() {
			// return newResource.exportRresource()
		// }
	},
