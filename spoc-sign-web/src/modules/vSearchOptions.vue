...
<template>
	<div class="v_search_options">
		<div class="v_search_options_content" :style="{'align-items':valign}">
			<div class="title" :style="{'min-width':_labelWidth+'px',textAlign:labelPosition,color:titColor}" v-if="!!title">{{title}}：</div>
			<div class="title" :style="{'min-width':_labelWidth+'px',textAlign:labelPosition,color:titColor}" v-else>{{title}}</div>
				...
				this.active1.forEach((v,k)=>{
					if(v!=''){
						this.list.forEach((val)=>{
							if(v==val.id){
								obj.childTags.push({id:v,title:val[this._key]})
							}
						})
					}
				})
...
//					this.multipleEmit();
			...
			companySelected:{
				handler:function(){
					if(this.companySelected.length){
						let arr=this.companySelected;
						this.active1=arr;
						this.multipleEmit();
					}
				},
