...
<template>
	<div>
		<div class="packsBox" v-for="(items,index) in data.policyData.list" :key="index">
			<Row type="flex">
				<Col span="8">
				<FormItem label="选择课程：" :prop="'list.'+index+'.itemId'" :rules="{required: true, message: '课程不能为空', trigger: 'change'}">
				...
				<Button type="ghost" @click="addBag" v-show="data.policyData.list.length==index+1">添加课程</Button>
				<Button type="ghost" @click="remBag(index)" v-show="data.policyData.list.length>1" style="margin-left: 20px;">删除课程</Button>
				</Col>
			</Row>
			<Row type="flex">
				<Col span="8">
				<FormItem label="课时数：" :rules="{required: true,pattern: /^\d+$/, message: '必须为整数', trigger: 'change'}">
					<Input v-model="items.giftCount" @on-change="numChange(items,$event)" style="width:262px;"><span slot="append">小时</span></Input>
				</FormItem>
				</Col>
				<Col span="8">
				<FormItem label="单价：">
						<Input v-model="items.unitPrice" disabled style="width:262px;"><span slot="append">元</span></Input>
				</FormItem>
				</Col>
				<Col span="8">
				<FormItem label="定价：">
	...
	export default{
		...
		methods: {
			itemIdChange(o) {
				this.data.htItemList.forEach(v => {
					if(v.id == o.itemId) {
						...
						o.publicPrice = this.accMul(v.giftCount,v.costPrice).toFixed(2);
						o.ratio='100';
						this.ratioChange(o);
					}
				})
			...
			numChange(o, e) {
				...
				o.publicPrice = this.accMul(o.giftCount, o.unitPrice).toFixed(2);
				this.ratioChange(o);
			},
			accMul(arg1, arg2) {
				let m = 0,
