...
<template>
	<div>
		<div class="packsBox" v-for="(items,index) in data.policyData.list" :key="index">
			<Row type="flex">
				<Col span="8">
				<FormItem label="选择计划：" :prop="'list.'+index+'.itemId'" :rules="{required: true, message: '托管服务不能为空', trigger: 'change'}">
			...
			<Row type="flex">
				...
				<FormItem label="卡片金额：">
					<div v-if="items.itemId">
						<Input v-for="item in data.htItemList" :key="item.id" v-model="item.htItemSettingMap.TechCourse" v-if="item.id==items.itemId" disabled style="width:262px;"><span slot="append">元</span></Input>
					</div>
					<div v-else>
		...
		methods: {
			itemIdChange(o) {
				this.data.htItemList.forEach(v => {
					if(v.id == o.itemId) {
						o.publicPrice = Number(v.htItemSettingMap.TechCourse).toFixed(2);
						this.ratioChange(o);
					}
				})
				this.$emit('on-change');
